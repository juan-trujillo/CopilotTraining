...
const deckTitle = computed(() => {
  ...
  const modulePath = frontmatter?.module || 'intro-talks'
  return `GitHub Copilot Training : ${modulePath}`
})
</script>

<template>
  <!-- GitHub Invertocat branding (bottom-left) -->
  <footer
    class="absolute bottom-3 left-4 flex items-center gap-2 select-none"
    style="z-index: 100;"
  >
    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="#8b949e" opacity="0.5">
    ...
    <span class="text-xs font-light tracking-wide" style="color: rgba(139, 148, 158, 0.4);">{{ deckTitle }}</span>
  </footer>

  <!-- Slide numbers (bottom-right) -->
  <footer
    v-if="currentPage && total"
    class="absolute bottom-3 right-4 text-xs select-none font-light"
    style="z-index: 100; color: rgba(139, 148, 158, 0.4);"
  >
    {{ currentPage }} / {{ total }}
  </footer>
</template>
